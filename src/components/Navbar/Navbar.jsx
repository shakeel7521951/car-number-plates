    ...
    <nav className='p-2 relative'>
      ...
      <div className='md:hidden'>
        ...
        >
          ...
          {/* Mobile "Post" Button */}
          {profile && profile?.role !== 'buyer' && (
            <div className='flex justify-start bg-white mt-4 items-start rounded'>
              <Link
                to={'/createPlate'}
                onClick={() => setIsMobileMenuOpen(false)}
                className='bg-transparent text-black font-bold w-max px-4 py-1 rounded flex items-center justify-between'
              >
                <FaPlus className='font-bold text-black me-2' />
                {language === 'eng' ? 'Post' : 'بريد'}
              </Link>
            </div>
          )}

          <div className='flex ml-6 mt-4 ms-0 w-[100%]'>
            <select
              id='language'
              value={language}
              onChange={handleLanguageChange}
              className='p-3 px-5 w-max rounded-lg bg-gray-800 text-white text-base font-semibold border border-gray-600 focus:outline-none transition-all duration-300 hover:bg-gray-700 cursor-pointer '
            >
              <option
                value='eng'
                className='cursor-pointer text-start text-[10px]'
              >
                English
              </option>
              <option value='arabic' className='cursor-pointer text-[10px]'>
                Arabic
              </option>
            </select>
          </div>
