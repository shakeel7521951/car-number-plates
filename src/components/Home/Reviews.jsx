...
const Reviews = () => {
  ...
  return (
    <div className="flex flex-col items-center gap-8 px-2 sm:px-8 font-inter mt-12">
      {/* Section Title */}
      <h2 className="text-white text-xl px-8 md:text-3xl lg:text:4xl font-semibold font-inter text-center">
        {language === "eng"
      ...
      {/* Reviews Grid */}
      <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-3 w-full max-w-[300px] md:max-w-[1582px] md:my-6">
        {reviewsData.map((testimonial) => (
          <div
            key={testimonial.id}
            className="text-white border-2 border-[#FFEA01B2] shadow-2xl rounded-[20px] p-2 md:p-6 flex flex-col items-center text-center transition-transform duration-300 ease-in-out hover:scale-[1.02] hover:shadow-[0_8px_16px_rgba(0,0,0,0.3)] "
          >
            {/* Five Stars */}
            <div className="my-2 md:my-4 text-yellow-500 flex text-xl text-md:3xl gap-2">
              {[...Array(5)].map((_, index) => (
                <FaStar key={index} />
              ))}
            </div>
            ...
            {/* Profile Picture */}
            <img
              className="w-[62px] h-[62px] md:w-[92px] md:h-[92px] rounded-full mb-4 object-cover"
            ...
            {/* Handle and Days Ago */}
            <div className="text-[12px] md:text-sm font-normal font-inter flex items-center justify-between gap-2">
              {testimonial.handle}{" "}
              <span>
                <GoDotFill size="8px" />
              </span>{" "}
            ...
            <div className="my-4 md:my-6">
              <img src={google} alt="Google" className="w-12 h-auto md:w-14" />
            </div>

            {/* Customer Quote */}
            <div className="text-[12px] px-5 md:px-0 sm:text-md md:text-[16px] font-light font-inter max-w-[420px]">
